# ACR1311U-N2 ETDA Companion Probe

Host-side reference for exercising the ETDA companion APDU profile defined in
[`docs/superpowers/specs/etda-nfc-companion-apdu.md`](../../docs/superpowers/specs/etda-nfc-companion-apdu.md).
...
1. `SELECT` ETDA AID `A0000004544410100`
...
Pinned constants: `src/config/etdaCompanionApdu.ts`.

## Script

`etda_companion_probe.ts` is a **stub** until the ACS SDK is wired for this repo.
Implement using the sequence in spec §11 when a physical reader is available.

```bash
# Future (after SDK binding):
# npx ts-node tools/acr1311u-n2/etda_companion_probe.ts
```
