...
describe('ScanSuccessPanel', () => {
  test('shows the post-confirm scan success document summary', () => {
    ...
    expect(screen.getByText('รับเอกสารสำเร็จ').props.className).toContain('leading-[26px]')
    expect(screen.getByText(/เอกสาร :/).props.className).toContain('leading-[22px]')
    expect(screen.getByText(/หน่วยงานที่รับรอง :/)).toBeTruthy()
    expect(screen.getByText(/หน่วยงานที่รับรอง :/).props.className).toContain('leading-[22px]')
    expect(screen.getByText(/ใบแสดงผลการเรียน/)).toBeTruthy()
    expect(screen.getByText(/มหาวิทยาลัยจุฬาลงกรณ์/)).toBeTruthy()
  })
})
