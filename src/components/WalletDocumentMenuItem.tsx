      ...
      {onViewOldCredential && oldCredentialLabel ? (
        <View className="pt-2">
          <Pressable onPress={onViewOldCredential} className="items-center py-1">
            <Text className="text-xs font-semibold text-[#002887]">{oldCredentialLabel}</Text>
          </Pressable>
        </View>
      ) : null}
      ...
      {isExpanded && inactivePanelMessage ? (
        <View className="items-center pt-3">
          <View className="h-12 w-12 items-center justify-center rounded-full border-2 border-wallet-navy">
            <MaterialCommunityIcons name="lock-outline" size={28} color="#002887" />
          </View>
          <Text className="mt-2 text-center text-xs text-[#4b5563]">{inactivePanelMessage}</Text>
