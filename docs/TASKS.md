...
- Added ADR 0007 and the Android-first native Ed25519 signing slice: new local Expo module `modules/etda-wallet-eddsa` wraps AndroidKeyStore Ed25519 generation/signing, normalizes public keys to raw 32-byte Ed25519 bytes, returns raw 64-byte signatures, checks `KeyInfo.securityLevel` for TEE/StrongBox after generation, and gates signing with Android `BiometricPrompt`. `src/services/crypto/crypto.ts` now derives an Ed25519 `did:key`, emits `alg: EdDSA` for OID4VCI PoP, OID4VP JWT VP, and SD-JWT KB-JWT tokens, and startup now hard-blocks when `nativeEddsaSigner` is unavailable. Removed the development-only software EdDSA signing flag/path from app code and `.env.example`, removed `@animo-id/expo-secure-environment` from dependencies, and updated architecture/security/spec roadmap docs. Remaining blocker: run Android prebuild/build and validate on the actual target Android device that AndroidKeyStore Ed25519 key generation works with TEE/StrongBox; iOS Ed25519 remains deferred.
- Tightened the Android Ed25519 native module support probe after first fresh-start testing: `supportsSecureEnvironment()` now requires Android hardware keystore feature version 200+ for Curve25519/Ed25519 and no longer treats a generic EC AndroidKeyStore generator as Ed25519 support. The connected Galaxy device reports Android SDK 36 and `pm has-feature android.hardware.hardware_keystore 200=true`; if startup still shows `NativeEd25519SignerRequired`, reinstall/rebuild the native dev app so the newly added `EtdaWalletEddsa` module is present in the binary.
