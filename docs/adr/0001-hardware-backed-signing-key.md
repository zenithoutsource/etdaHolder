...
On 2026-08-13, ADR 0011 restored this ADR's hardware-non-extractable principle for production holder keys using P-256 / ES256 in AndroidKeyStore (`k_attest` and `k_cred`). Ed25519 `k_cred` remains only behind `EXPO_PUBLIC_HARDWARE_P256_SIGNING_ENABLED=false`.
