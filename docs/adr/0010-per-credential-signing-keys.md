...
ADR 0008 remains the algorithm and storage baseline: Keychain-protected software Ed25519, biometric sign-time gate, `alg: EdDSA` on the wire. Hardware WSCD / non-extractable keys are out of scope until target Android devices prove Ed25519 in AndroidKeyStore.

## Decision

Adopt **v2 crypto** with two key classes, shipped together:

1. **`k_attest`** — one wallet attestation key (`wallet.ed25519_seed.attest`) for Wallet Provider WUA/WIA. Cached attestations in MMKV (`wallet.attest.wua`, `wallet.attest.wia`). v2 issuance is gated until activation attest succeeds (`wallet.crypto.v2_enabled`).
...
3. **Hardware WSCD per key** — strongest isolation; blocked on Galaxy A26 Ed25519 KeyStore validation. Deferred.
...
- ADR 0008 — Keychain-protected Ed25519 production signing (algorithm unchanged)
