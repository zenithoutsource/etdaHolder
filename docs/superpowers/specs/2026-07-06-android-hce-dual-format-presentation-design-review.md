...
เอกสารขึ้นสถานะ **Approved** พร้อมข้อจำกัดชัด: implement dual-format NFC ยังต้องรอ **the customer companion APDU spec** และ **EdDSA interop test pass** ก่อน
...
1. **ขอบเขตมาตรฐาน** — แยก ISO 18013-5 กับ the customer extension; OID4VCI/OID4VP/dual NFC ชัด
...
4. **Pre-tap request resolution** — fixed the customer reader profile ใน `src/config/`; approve = ceiling
...
| Dependency | หมายเหตุ |
|---|---|
| **the customer companion APDU spec** | AID, CLA/INS, nonce — ต้องมีก่อน implement dual-format NFC |
...
1. **Config ที่ต้องสร้างตอน implement** — the customer reader profile (`src/config/`), `EXPO_PUBLIC_DUAL_FORMAT_ISSUE_SKEW_MS`, `EXPO_PUBLIC_HCE_ARM_WINDOW_MS`, `EXPO_PUBLIC_NFC_PAYLOAD_MAX_BYTES` ยังไม่มีใน `.env.example` จนกว่าจะลงมือ implement (spec กำหนดไว้แล้ว)
...
1. Track ใน `docs/TASKS.md`: the customer companion APDU spec, dual-format issuance, proximity refactor (consent-first), OID4VP multi-format
2. หลังได้ reader จริง: รัน EdDSA interop ก่อน — ตาม test matrix
3. หลัง interop: ADR เลือก mDOC native module
4. Implement the customer reader profile ใน `src/config/` คู่กับ ACR1311U-N2 template ที่ lock
